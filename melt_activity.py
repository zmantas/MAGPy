...
class melt_activity():
	...
	def __init__(self,T,system):
		...
		# Setting inital gamma values to 1
		self.gamma = {}
		for ox in system._oxideNames: 
			self.gamma[ox] = 1
	...
	def recompute_gamma(self,act_ox,addF2O3):
		...
		elif addF2O3 and act_ox['Fe2O3'] != 0 and fAbMolecule['Fe'] != 0:
	...
	def melt_activity_calculation(self,sim,addF2O3=False):
		...
		while iit < 1e8: 
			...
			fGam = {}
			for oxide in self.gamma: 
				if gamma_new[oxide] == 0 or \
				   (oxide == 'Fe2O3' and gamma_new['FeO'] == 0):
					fGam[oxide] = 1
				else:
					fGam[oxide] = gamma_new[oxide]/self.gamma[oxide]
			...
			# 	         gamma_new[oxide]/self.gamma[oxide]\
			...
			for element in gamma_new:
				if iit > 500:
					self.gamma[element] = (gamma_new[element] * self.gamma[element]**4)**(1/5)
				elif iit > 30:
					self.gamma[element] = (gamma_new[element] * self.gamma[element]**2)**(1/3)
				else:
					self.gamma[element] = (gamma_new[element] * self.gamma[element])**(1/2)
